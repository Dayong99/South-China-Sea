// 获得canvas
export default function getStationCanvas(device) {
    const canvas = document.createElement('canvas')
    const ctx = canvas.getContext('2d')
    canvas.width = 80
    canvas.height = 80
    ctx.font = '700 14px MingLiU'
    // 参数位置
    let firstParmsX = 10 * (Math.sin((0) * Math.PI / 180)).toFixed(1)
    let firstParmsY = 10 * (Math.cos((0) * Math.PI / 180)).toFixed(1)
    let secondParmsX = 10 * (Math.sin((360 / 5) * Math.PI / 180)).toFixed(1)
    let secondParmsY = 10 * (Math.cos((360 / 5) * Math.PI / 180)).toFixed(1)
    let thirdParmsX = 10 * (Math.sin((360 / 5 * 2) * Math.PI / 180)).toFixed(1)
    let thirdParmsY = 10 * (Math.cos((360 / 5 * 2) * Math.PI / 180)).toFixed(1)
    let fourthParmsX = 10 * (Math.sin((360 / 5 * 3) * Math.PI / 180)).toFixed(1)
    let fourthParmsY = 10 * (Math.cos((360 / 5 * 3) * Math.PI / 180)).toFixed(1)
    let fifthParmsX = 10 * (Math.sin((360 / 5 * 4) * Math.PI / 180)).toFixed(1)
    let fifthParmsY = 10 * (Math.cos((360 / 5 * 4) * Math.PI / 180)).toFixed(1)
    // 绘制风羽
    drawWind(ctx, device, canvas.width, canvas.height)
    ctx.beginPath();
    ...
    ctx.fillText('' + device.temperature, canvas.width / 2 + firstParmsX - 7,
        canvas.height / 2 + firstParmsY + 10)
    // 参数2
    ctx.fillStyle = '#289434'
    ctx.fillText('' + device.humidity, canvas.width / 2 + secondParmsX + 4,
        canvas.height / 2 + secondParmsY + 2)
    // 参数3
    ctx.fillStyle = '#3a24ed'
    ctx.fillText('' + device.visibility, canvas.width / 2 + thirdParmsX,
        canvas.height / 2 + thirdParmsY - 4)
    // 参数4
    ctx.fillStyle = '#e01549'
    ctx.fillText('' + device.accumulatedRainfall, canvas.width / 2 + fourthParmsX - 10,
        canvas.height / 2 + fourthParmsY - 4)
    // 参数5
    ctx.fillStyle = '#1b5cf7'
    ctx.fillText('' + device.pressure, canvas.width / 2 + fifthParmsX - 20,
        canvas.height / 2 + fifthParmsY + 2)
    return canvas
}
...
function drawWind(ctx, windObj, width, height) {
    ...
    let len = 24 // 风向线长度 16
    let r = 4 // 风力线长度 4
    let arc = Math.PI / 180 * (windObj.instantaneousWindDirection - 180) // 旋转角
    let a = startPoint.x
    let b = startPoint.y
    let x0 = a
    ...
    let level = getLevel(windObj.instantaneousWindSpeed) // 根据风速设置风级   横杠个数
