<template>
  ...
  >
    ...
    <!-- <div
      class="save_wrapper"
      v-if="title === '添加色斑图' || title === '修改色斑图'"
    >
      <button class="save" type="button" @click="createColor()">生 成</button>
    </div> -->
    ...
    <div v-show="colorShow">
      ...
      >
        <el-row :gutter="20">
          ...
          <el-col :span="8" class="color_show insert_color">
            <el-form-item
              label="插入颜色"
              prop="color"
              style="position: relative"
            >
              <el-input v-model="insertObj.color" />
              <el-color-picker
                class="color_value"
  ...
  methods: {
      ...
      if (this.colorIndex !== null) {
        this.$confirm(
          "是否确定删除以下颜色：<span style='background:" +
            this.gradientColorArr[this.colorIndex] +
    ...
    submitForm() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          if (this.color.colorValues.length === 0) {
            this.$message.warning("图例值不能为空");
          } else if (this.title === "添加图例") {
            // 新增
